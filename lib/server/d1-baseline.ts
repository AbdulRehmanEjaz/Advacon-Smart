...
export function baselineSql(timestamp = new Date().toISOString()) {
  return [
    ...
    `INSERT OR IGNORE INTO users (id,name,role,active,created_at,updated_at) VALUES ${row(['initial-admin', 'Project Administrator', 'ADMIN', 1, timestamp, timestamp])},${row(['initial-foreman', 'Site Supervisor', 'FOREMAN', 1, timestamp, timestamp])},${row(['initial-viewer', 'Project Viewer', 'VIEWER', 1, timestamp, timestamp])};`,
