...
function DocumentSection({ kind, items, subtotal, preview, onAdd, onEdit, onArchive }: { kind: CostDocumentType; items: InvoicePoRecord[]; subtotal: number; preview: boolean; onAdd: () => void; onEdit: (item: InvoicePoRecord) => void; onArchive: (id: string) => void }) {
  ...
  return <section className="card cost-section cost-document-section">
    ...
    {!items.length ? <p className="cost-empty">No {plural.toLowerCase()} recorded for this month.</p> : <div className="cost-records">{items.map((item) => <article key={item.id}>
...
export function CostControlPage({ state, refresh, preview }: { state: State; refresh: () => Promise<void>; preview: boolean }) {
  ...
  return <div className="cost-page">
    <section className="card cost-hero"><div><span className="eyebrow">FINANCIAL CONTROL</span><h2>Total Recorded Project Cost — Non-VAT</h2><p>Live attendance-derived costs plus recorded Fuel, Invoice and PO net amounts.</p></div><label className="field"><span>Reporting month</span><input type="month" max={currentMonth} value={month} onChange={(event) => setMonth(event.target.value)} /></label></section>
    ...
    <section className="card cost-summary-card"><div><Droplets /><span>Authoritative monthly formula</span><strong>Manpower + Equipment + Fuel Net + Invoices Net + POs Net</strong></div><strong><Money value={summary.totalHalalas} /></strong></section>
